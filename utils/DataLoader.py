...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float, supervised: bool):
    """
    ...
    assert node_raw_features.shape[1] == edge_raw_features.shape[1], 'Unaligned feature dimensions'
    ...
    if supervised : 
        ...
        full_data = Data(
            src_node_ids=src_node_ids_test,
        ...
        train_mask = np.arange(len(src_node_ids_test)) <= int((1-val_ratio-test_ratio) * len(src_node_ids_test))

        train_data = Data(
            src_node_ids=src_node_ids_test[train_mask], 
            dst_node_ids=dst_node_ids_test[train_mask],
            node_interact_times=node_interact_times_test[train_mask],
            edge_ids=edge_ids_test[train_mask], 
            labels=labels_test[train_mask],
            packet_id=packet_ids_test[train_mask],
            attack_type=attack_type_test[train_mask]
        )

        # Val data
        val_mask = np.logical_and(
            np.arange(len(src_node_ids_test)) > int((1 - val_ratio - test_ratio) * len(src_node_ids_test)),
            np.arange(len(src_node_ids_test)) <= int((1 - val_ratio) * len(src_node_ids_test))
        )

        val_data = Data(
            src_node_ids=src_node_ids_test[val_mask], 
            dst_node_ids=dst_node_ids_test[val_mask],
            node_interact_times=node_interact_times_test[val_mask],
            edge_ids=edge_ids_test[val_mask], 
            labels=labels_test[val_mask],
            packet_id=packet_ids_test[val_mask],
            attack_type=attack_type_test[val_mask]
        )
        
        # Test data
        test_mask = np.arange(len(src_node_ids_test)) > int((1 - val_ratio) * len(src_node_ids_test))
        
        test_data = Data(
            src_node_ids=src_node_ids_test[test_mask], 
            dst_node_ids=dst_node_ids_test[test_mask],
            node_interact_times=node_interact_times_test[test_mask],
            edge_ids=edge_ids_test[test_mask], 
            labels=labels_test[test_mask],
            packet_id=packet_ids_test[test_mask],
            attack_type=attack_type_test[test_mask]
        )
        ...
        full_data = Data(
            src_node_ids=np.concatenate([src_node_ids, src_node_ids_test], axis=0), 
            dst_node_ids=np.concatenate([dst_node_ids, dst_node_ids_test], axis=0), 
            node_interact_times=np.concatenate([node_interact_times, node_interact_times_test], axis=0), 
            edge_ids=np.concatenate([edge_ids, edge_ids_test], axis=0), 
            labels=np.concatenate([labels, labels_test], axis=0),
            packet_id=np.concatenate([packet_ids, packet_ids_test], axis=0),
            attack_type=np.concatenate([attack_type, attack_type_test], axis=0)
        )
        ...
        train_data = Data(
            ...
            packet_id=packet_ids[train_mask],
            attack_type=attack_type[train_mask]
        )

        # val data
        val_mask = np.arange(len(src_node_ids)) > int((1-val_ratio) * len(src_node_ids))
        ...
        val_data = Data(
            ...
            packet_id=packet_ids[val_mask],
            attack_type=attack_type[val_mask]
        )
